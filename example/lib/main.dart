  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      home: Scaffold(
        body: IndexedStack(
          index: _index,
          children: List.generate(_pages.length, (i) => _pages[i] ?? const SizedBox.shrink()),
        ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        padding: const EdgeInsets.all(20),
        children: [
          _row('设备型号', DefaultBRWebCapabilityHandler.systemInfo?.deviceModel ?? 'N/A'),
          _row('系统', '${DefaultBRWebCapabilityHandler.systemInfo?.os ?? ''} ${DefaultBRWebCapabilityHandler.systemInfo?.osVersion ?? 'N/A'}'),
          _row('App 版本', DefaultBRWebCapabilityHandler.systemInfo?.appVersion ?? 'N/A'),
          _row('Build', DefaultBRWebCapabilityHandler.systemInfo?.buildNumber ?? 'N/A'),
          _row('模拟器', '${DefaultBRWebCapabilityHandler.systemInfo?.isEmulator ?? false}'),
        ],
      ),
    );
  }
